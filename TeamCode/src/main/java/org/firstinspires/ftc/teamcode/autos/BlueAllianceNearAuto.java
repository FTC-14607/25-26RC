...
public class BlueAllianceNearAuto extends LinearOpMode {
    ...
    // Turning (degrees)
    public static double TURN_RIGHT_DEG = 33;
    ...
    public void runOpMode() {
        ...
        showStep("6) rotate right " + TURN_RIGHT_DEG  + " deg");
        robot.rotate(TURN_RIGHT_DEG);
        if (!opModeIsActive()) return;

        //8: strafe left
        showStep("7) strafe left " + STRAFE_1_RIGHT_IN + " in");
        robot.right(STRAFE_1_RIGHT_IN, DRIVE_SPEED_TPS);
        if (!opModeIsActive()) return;
        ...
        robot.right(FINAL_STRAFE_RIGHT_IN, DRIVE_SPEED_TPS);
        */
        // End: brake motors
        showStep("DONE");
        robot.brake();
